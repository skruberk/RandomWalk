...
% Define the predefined distance from the origins of inner circles
origin_check=target*1.5
...
for run = 1:num_runs
    ...
    for i = 2 :tsteps
        ...
        % Iterate through all the inner circle origins
        for j = 1:num_discs
            % Check if the random walk is within the predefined distance from the origin of the k-th inner circle
            if norm([xbound, ybound] - org_coord(j, :)) <= origin_check
                % Check if it's at any of the inner circles
                for k = 1:num_discs  % Checks if particle is at any target     
